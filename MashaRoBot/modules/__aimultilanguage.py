...
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            await lel.edit("lovely AI Already Activated In This Chat")
            return
        await lel.edit(
            f"lovely AI Successfully Added For Users In The Chat {message.chat.id}"
        )
...
async def hmm(client, message):
    ...
    if chat_id in en_chats:
        ...
        test = test.replace("lovely", "Aco")
        test = test.replace("lovely", "Aco")
        URL = f"https://api.affiliateplus.xyz/api/chatbot?message={test}&botname=@lovelyr_obot&ownername=@aboutvedmat"
        try:
            r = requests.request("GET", url=URL)
        except:
            return
